function [finalResults,binomVector,NoOfSuccesses] = ThreeCombEnsembleClassifier(ThreeCombClassifiersList,testHOGFeat, testColorFeat, testGISTFeat,testSIFTFeat,test_labels)
tic    
finalResults = [];
...
for i=1:length(test_labels)
    ...
    equalVotes = 0;
     %for HOG feature
     for j=1:length(ThreeCombClassifiersList)
         if j == 1    %for Color feature
             [classPrediction,predProScore] = predict(ThreeCombClassifiersList{j},testGISTFeat(i,:));
...
%          elseif j==2 %for color feature
%               [classPrediction,predProScore] = predict(ThreeCombClassifiersList{j},testHOGFeat(i,:));
%       
%             if strcmp(classPrediction{1},'Kangaroo')==1                %ned to add weights
%                 kangaroosCount = kangaroosCount + 1;
%              else 
